...
def main():
    ...
    run_check("SeqKit", f"seqkit stats {raw}/test_reads_R1.fq")

    # 2. Alignment Tools (DNA)
    # Bowtie2
    run_check(
        "Bowtie2 Index", f"bowtie2-build {refs}/tiny_genome.fa {refs}/tiny_genome"
    )
    ...
    # BWA
    run_check("BWA Index", f"bwa index {refs}/tiny_genome.fa")
    ...
    # 7. Bedtools
    run_check(
        "Bedtools GenomeCov", f"bedtools genomecov -ibam {res}/bowtie2.sorted.bam", None
    )
